...
class GUI():
    ...
    def mainloop(self):
        ...
        self.label2.grid(row=2, column=0, columnspan=6)

        self.label3 = Label(self.master, text='')
        self.label3.grid(row=2, column=7, columnspan=6)
        ...
        self.stop_s.grid(row=0, column=7, columnspan=2, ipadx=50, ipady=50, rowspan=2,
                         padx=25, pady=25)
    ...
    def __action(self):
        ans = action()
        self.label2.config(text=ans[0])
        self.label3.config(text=ans[1])
